...
async def ingest_transaction(payload: TransactionIngest):
    ...
    try:
        ...
        route_key = (
            f"route:{payload.issuing_bank}:{payload.pos_provider}:{payload.card_type}"
        )

        is_hard_error = "1" if payload.response_code in HARD_ERROR_CODES else "0"
        is_ghost = "1" if payload.ghost_debit else "0"

        member = f"{payload.id}:{is_hard_error}:{is_ghost}:{payload.card_type}"
...
@app.get("/api/v1/routes/health")
async def get_routes_health():
    route_keys = await redis_client.keys("route:*:*:*:status")
    health_data = []

    for status_key in route_keys:
        parts = status_key.split(":")
        if len(parts) >= 4:
            bank = parts[1]
            pos_provider = parts[2]
            card_type = parts[3]

            status_hash = await redis_client.hgetall(status_key)

            fail_rate_pct = float(status_hash.get("fail_rate_pct", 0.0))
            failure_rate = fail_rate_pct / 100.0  # Decouple to decimal

            health_data.append(
                {
                    "route_key": status_key,
                    "bank": bank,
                    "pos_provider": pos_provider,
                    "card_type": card_type,
                    "status": status_hash.get("status", "HEALTHY"),
                    "failure_rate": failure_rate,
                    "fail_rate_pct": fail_rate_pct,
                    "avg_latency_ms": float(status_hash.get("avg_latency_ms", 0.0)),
                    "volume_5m": int(status_hash.get("volume_5m", 0)),
                }
            )

    return {"routes": health_data}


@app.get("/api/v1/incidents/active")
async def get_active_incidents():
    """
    Retrieves unresolved alerts or recent incident logs generated by evaluator node and LLM agent.
    """
    try:
        raw_incidents = await redis_client.lrange("active_incidents", 0, 10)
        incidents = [json.loads(inc) for inc in raw_incidents] if raw_incidents else []
        return {"status": "success", "count": len(incidents), "incidents": incidents}
    except Exception as e:
        sentry_sdk.capture_exception(e)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to fetch active incidents: {e!s}",
        )
